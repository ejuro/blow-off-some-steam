...
BASE = pathlib.Path('/tmp/steam-perf-review')
REPO = pathlib.Path('/home/je93/Work/omarchy-plugins/blow-off-some-steam')
...
def prepare(name):
    ...
    if name in ('destruction','falling','destruction_no_effects','destruction_no_terrain'):
        ...
        arena.desktopSnapshot = "file:///tmp/steam-perf-review/snapshot.ppm";
...
results=[]
names=['holstered','idle','aim','aim_no_effects','aim_dirty','aim_small_canvas','auto','auto_no_effects','rockets','destruction']
...
if args.scenarios: names=args.scenarios.split(',')
